...
class Category extends React.Component {
  ...
  render() {
    ...
    const ShowsByCat = showsByCategory.map((show) => <ShowCard key={show.CategoryId} show={show} showDate={show.showDateTime} showTime={show.showDateTime} />)
    return (
     <div>
       <h1 className="text-center">This is the Category Page</h1>
       <h2>do this work?</h2>
       <div className="show-by-cat">{ShowsByCat}</div>
     </div>
    )
  }
}
